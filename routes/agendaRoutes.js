...
router.get('/aluno/:cod', (req, res) => {
  ...
  db.query(sql, [codAluno], (err, results) => {
    ...
    function renderizarAgenda() {
      const sqlRecados = `SELECT cod, descricao, DATE_FORMAT(datar, "%Y-%m-%d") AS data, "Recado" AS tipo FROM recados WHERE agenda_id = ?`;
      const sqlEventos = `SELECT cod, descricao, DATE_FORMAT(datae, "%Y-%m-%d") AS data, "Evento" AS tipo FROM eventos WHERE agenda_id = ?`;
